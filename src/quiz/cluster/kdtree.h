...
struct KdTree
{
  	...
  	void insertHelper(Node *&node, uint depth, std::vector<float> point, int id)
    {
  	...
  	// Insert helper using double poinrt 
  	void insertHelperDp(Node** node, uint depth, std::vector<float> point, int id)
    {
      if(*node == NULL)
        *node = new Node(point, id);
      else
      {
        ...
        if(point[cd] < ((*node)->point[cd]))
          insertHelperDp(&((*node)->left), depth+1, point, id);
        else
          insertHelperDp(&((*node)->right), depth+1, point, id);
      }
    }
	...
	void insert(std::vector<float> point, int id)
	{
		...
		// the function should create a new node and place correctly with in the root
      	...
      	insertHelperDp(&root, 0, point, id);
	}
